...
<img src="eu_ai_act_auditor_architecture.png" alt="LLM Auditor Architecture" width="800"/>
...
2.  **Installation**
    ...
    cd adk-samples/python/agents/eu_ai_act_auditor
...
You may open the URL, select "eu_ai_act_auditor" in the top-left drop-down menu, and
...
[risk_classification_agent]: Okay, I will double-check the statement: "Earth is further away
...
[data_governance_agent]: Mars is further away from the Sun than Earth.
```
...
[risk_classification_agent]: Okay, I will analyze the answer to the question "Why the
...
[data_governance_agent]: Blueberries are blue due to a waxy coating on their skin that
...
Then the tests and evaluation can be run from the `eu_ai_act_auditor` directory using
the `pytest` module:
